...
namespace ch4_과제4
{
    class Program
    {
        ...
        {
            int[,] arrA = new int[20, 9]; // 수열을 저장할 2차원 배열
            int k, m, l;
            int[,] cnt = new int[20, 5]; // 같은 숫자가 몇번 나왔는지 세는 배열
            int[,] newNum = new int[20, 4]; //새로운 숫자를 만났을 때 그 숫자를 저장 할 변수
            ...
            for (int i = 0; i < 20; i++)
            {
                ...
                while (k < 9 && arrA[i, k] != 0)                      //1. 배열 인덱스 범위 지정 2. null을 만나면 반복문 종료 - 수열을 읽는 부분
                {
                    ...
                    //{
                    //}
                    if ((cnt[i, m] > 1) || (((arrA[i, k + 1] != 0) && (arrA[i, k] != arrA[i, k + 1]))))    //현재 인덱스와 다음 인덱스의 값이 다르면 newNum을 새롭게 바꿔주고 m 1증가 시켜서 cnt배열 바꿔주기
                    {
                ...
                k = 1;
                while ((i < 19 && k < 9 && l < 2) && newNum[i, l] != 0)
                {
                    if (k % 2 == 0)
                    {
